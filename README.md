...
The maintained compatibility image is `ghcr.io/pasturestack/kubernetes-package:v1.12.10-pasturestack.4`. It rebuilds the Kubernetes 1.12.10 kubelet with narrowly scoped Docker 29, cgroup v2, containerd snapshotter, and sandbox IPC compatibility patches while retaining the verified upstream control-plane binaries. On a unified cgroup v2 host it reports CPU, memory, I/O, and writable-layer usage without reading the removed graphdriver `layerdb`. It also converges the Helm 2 compatibility server on `ghcr.io/pasturestack/tiller:v2.17.0-pasturestack.2` without deleting existing release records. Personal registry coordinates and inherited CI/CD have been removed.

The image has passed binary-version checks, Go 1.10 and Go 1.26 builds, shell validation, an Ubuntu 26.04 and Docker 29 static-Pod lifecycle test, direct loopback CNI add/delete, repeated cgroup v2 metric comparisons against Docker, Azure CLI dependency and vulnerable-API reachability checks, HIGH/CRITICAL vulnerability blocking, secret scanning, private-data checks, and CycloneDX SBOM generation. A complete Catalog release remains gated on multi-service control-plane, worker, add-on, upgrade, and rollback testing.
...
make package \
  ...
  TAG=v1.12.10-pasturestack.4 \
  KUBERNETES_BINARY_VERSION=v1.12.10-pasturestack.4
```

The command verifies the upstream server archive, extracts it into the legacy `k8s.io/kubernetes` GOPATH layout required by Kubernetes 1.12, applies the documented compatibility patch, builds the kubelet with `-trimpath`, creates a local image, and records its name in `dist/images`. The build rejects a kubelet that embeds its randomized temporary build directory so independent builds can be compared by digest. Both build and runtime stages use the reviewed Ubuntu 26.04 amd64 digest. Because that minimal base does not contain a CA bundle, the first APT transaction is restricted to `ca-certificates` and dependencies authenticated by Ubuntu's signed archive metadata; the build then discards those indexes, switches to the HTTPS package mirror, and installs every remaining package. The command does not push the image.

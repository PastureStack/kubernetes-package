...
TILLER_IMAGE=${TILLER_IMAGE:-tiller:v2.17.0-pasturestack.1}
...
# Helm 2 release records are cluster data. Update Tiller in place and preserve
# the complete pre-upgrade record set; never use replace --force for this path.
...
function converge_tiller() {
  ...
  local release_snapshot_dir release_count_before release_count_after missing_count
  ...
  if [ "${missing_count}" -ne 0 ]; then
    echo "Tiller update verification found ${missing_count} missing Helm 2 release record(s); restoring the previous Deployment revision" >&2
    restore_tiller_resources "${deployment_existed}" "${old_generation}" "${service_existed}" \
      "${service_account_existed}" "${role_existed}" "${binding_existed}"
    rm -rf "${release_snapshot_dir}"
    return 1
  ...
  echo "Tiller is ready at ${desired_image}; Helm 2 release records before=${release_count_before} after=${release_count_after} missing=0"
  rm -rf "${release_snapshot_dir}"
}

...
- Rebuild the CNI plug-ins `v1.9.1` loopback executable twice from its checksum-locked official source commit with Go 1.26.6, require byte-identical output, and retain the verified Apache-2.0 license in the image. Reject the inherited prebuilt archive and unexpected compiler metadata.
...
- Keep the Azure CLI Python dependency set internally consistent. Azure CLI core 2.89.1 requires MSAL 1.36.0, and the supported MSAL and pyOpenSSL set requires `cryptography <49`. The image therefore installs checksum-verified `cryptography 48.0.1` and `pyOpenSSL 26.2.0` wheels without invoking a network package resolver. Exact distribution checks, `pip check`, expiring reviewed vulnerable-API reachability checks, and the Azure command paths used by this package are release-blocking. Move to the fixed cryptography series only when a published Azure CLI dependency set supports it.
...
- [`CVE-2026-69249`](https://github.com/pyca/cryptography/security/advisories/GHSA-jwv3-5hgf-82ww) covers exponential X.509 path building. Its GitHub package range currently says `<=48.0.0`, while the [CVE CNA record](https://www.cve.org/CVERecord?id=CVE-2026-69249) says `<49.0.0`. The release gate conservatively treats exact `cryptography 48.0.1` as affected and fails if any installed consumer references `cryptography.x509.verification`, `PolicyBuilder`, `build_client_verifier`, or `build_server_verifier`.
- [`CVE-2026-69247`](https://github.com/pyca/cryptography/security/advisories/GHSA-g6cj-pr64-35w5) affects PKCS#7 `EnvelopedData` decryption APIs in `cryptography >=44,<50`. The image does not expose a PKCS#7 decryption service, and the same gate finds no installed consumer call to `pkcs7_decrypt_der`, `pkcs7_decrypt_pem`, or `pkcs7_decrypt_smime`.

Both exact-package VEX statements use `vulnerable_code_not_in_execute_path` and expire at `2026-09-15T00:00:00Z`. A build on or after that instant fails until the dependency constraint and reachability review are refreshed. These findings are accepted, not fixed; a supported Azure CLI dependency set that permits patched cryptography remains the required remediation.

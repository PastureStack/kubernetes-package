...
if [[ "$*" == *"get configmaps,secrets"* ]]; then
    config_value='Y29uZmlnLXZhbHVl'
    secret_value='c2VjcmV0LXZhbHVl'
    if [ -f "${KUBECTL_FIXTURE_APPLIED}" ] && [ "${KUBECTL_FIXTURE_RELEASE_DRIFT:-none}" = changed ]; then
        secret_value='Y2hhbmdlZC12YWx1ZQ=='
    fi
    cat <<JSON
{
  "apiVersion": "v1",
...
grep -Fq 'image: ghcr.io/pasturestack/tiller:v2.17.1' \
    "${fixture_root}/addons-success/helm/tiller-deploy.yaml"
grep -Fq 'serviceAccountName: "pasturestack-tiller"' \
    "${fixture_root}/addons-success/helm/tiller-deploy.yaml"
if ! grep -Fq 'release records before=2 after=2 changed-or-missing=0 changed-or-added=0' \
    "${fixture_root}/output.log"; then
    cat "${fixture_root}/output.log" >&2
    exit 1
fi

: > "${fixture_root}/kubectl-drift.log"
rm -f "${KUBECTL_FIXTURE_APPLIED}" "${KUBECTL_FIXTURE_UNDONE}"
export KUBECTL_FIXTURE_LOG="${fixture_root}/kubectl-drift.log"
export KUBECTL_FIXTURE_RELEASE_DRIFT=changed
unset KUBECTL_FIXTURE_FAIL_ROLLOUT
if bash "${repo_root}/package/addons-update.sh" > "${fixture_root}/drift-output.log" 2>&1; then
    echo 'TILLER_UPGRADE_FIXTURE_EXPECTED_RELEASE_DRIFT_FAILURE' >&2
    exit 1
fi
grep -Fq 'rollout undo deployment/tiller-deploy' "${fixture_root}/kubectl-drift.log"
grep -Fq 'changed-or-missing=1 changed-or-added=1 Helm 2 release record(s)' \
    "${fixture_root}/drift-output.log"
...
unset KUBECTL_FIXTURE_RELEASE_DRIFT
...
echo 'KUBERNETES_PACKAGE_TILLER_UPGRADE_FIXTURE_OK records=2 content=verified rollback=verified'
